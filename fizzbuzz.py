...
def binary_encoding(x, num_digits):
    ...
    return np.array(
        [x >> i & 1 for i in range(num_digits)]
    )  # pass in number x, we want a num_digits-bit number, loop through all the bits,  right-shifts
...
# do same as in franke.py, but use classifier instead of regressor
...
model = tf.keras.models.Sequential(
    [
        ...
        tf.keras.layers.Dropout(
            0.2
        ),  # 20% it might turn off a neuron, prevents overfitting somehow?
